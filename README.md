...
- `aidataendpoint.csv` — flat table
- `summary.txt` — rollup + list of confirmed endpoints
- `poc_candidates.txt` — **triage file for reporting**: confirmed + suspected hits only, each with its evidence, a copy-paste `curl` PoC (MCP hits get the JSON-RPC `initialize` POST), and any catch-all warning. This is where to look for PoC URLs — `summary.txt` lists only *confirmed* hits, so when a run has 0 confirmed but N suspected, the candidates are here. Disable with `--no-poc`.
